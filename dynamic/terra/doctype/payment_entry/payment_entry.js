...
frappe.ui.form.on('Payment Entry', {
	...
	setup: function(frm) {
		...
		frm.set_query("reference_doctype", "references", function() {
			if (frm.doc.party_type == "Customer") {
				var doctypes = ["Sales Order","Quotation", "Sales Invoice", "Journal Entry", "Dunning"];
		...
		frm.set_query("reference_name", "references", function(doc, cdt, cdn) {
			...
			if(child.reference_doctype == "Quotation") {
				filters["quotation_to"] = doc.party_type;
			...
			callback: function(r, rt) {
				if(r.message) {
					...
					$.each(r.message, function(i, d) {
						...
						if(!in_list(["Sales Order", "Quotation" ,"Purchase Order", "Expense Claim", "Fees"], d.voucher_type)) {
	...
	validate_reference_document: function(frm, row) {
		var _validate = function(i, row) {
			...
			if(frm.doc.party_type=="Customer" &&
				!in_list(["Sales Order","Quotation", "Sales Invoice", "Journal Entry", "Dunning"], row.reference_doctype)
			) {
