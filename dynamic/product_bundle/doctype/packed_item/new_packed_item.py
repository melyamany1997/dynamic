	...
	for item_row in doc.get("items"):
		...
		#add data to child table 
			...
			for bundle_item in get_product_bundle_items(item_row.item_code):
				# check if bundel is complicated 
				if not  is_product_bundle(bundle_item.item_code) :
				...
				# if pundel is complicated add child pundels to backed list 
				if   is_product_bundle(bundle_item.item_code) :
					for small_bundle_item in get_product_bundle_items(bundle_item.item_code):
						pi_row = add_packed_item_row(
							doc=doc,
							packing_item=small_bundle_item,
							main_item_row=item_row,
							packed_items_table=stale_packed_items_table,
							reset=reset,
							)
						
						item_data = get_packed_item_details(small_bundle_item.item_code, doc.company)
						
						update_packed_item_basic_data(item_row, pi_row, small_bundle_item, item_data ,small_bundle_item.item_code ,bundle_item.qty)
						update_packed_item_stock_data(item_row, pi_row, small_bundle_item, item_data, doc)
						update_packed_item_price_data(pi_row, item_data, doc)
						update_packed_item_from_cancelled_doc(item_row, small_bundle_item, pi_row, doc)
...
def get_indexed_packed_items_table(doc):
	...
	for packed_item in doc.get("packed_items"):
		key = (packed_item.parent_item, packed_item.item_code, 
							packed_item.parent_detail_docname ,packed_item.prevdoc_doctype)
...
def update_packed_item_basic_data(main_item_row, pi_row, packing_item, item_data , perv =None , perf =None):
	...
	pi_row.qty = flt(packing_item.qty) * flt(main_item_row.stock_qty) *float(perf or 1)
	pi_row.conversion_factor = main_item_row.conversion_factor
	#to make key in case of child pundels
	#edited By Beshoy -- >to resolve un added item in case of bundel of bundels 
	pi_row.prevdoc_doctype = perv if perv else  "master"
