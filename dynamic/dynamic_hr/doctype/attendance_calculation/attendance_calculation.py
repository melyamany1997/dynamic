...
class AttendanceCalculation(Document):
	...
	def calculate_logs(self):
		emp_conditions = ""
		if len(getattr(self,'employees' , [])) > 0 :
			emp_conditions += "and emp.name in ({})".format(','.join([f"'{x[0]}'" for x in self.employees]))
		# frappe.msgprint(emp_conditions)
		from_date = self.from_date + timedelta(-1)
		logs_sql = f"""
		select log.* , emp.name as employee from tabEmployee emp 
		inner join `tabDevice Log` log on log.enroll_no = emp.attendance_device_id
			where date(log_time) between date('{from_date}') and date('{self.to_date}') {emp_conditions}
			order by emp.name asc, log_time asc
		"""
		# frappe.msgprint(logs_sql)
		logs = []
		logs = frappe.db.sql(logs_sql,as_dict=1) or []
		self.employees_logs = {}
		for emp in getattr(self,'employees' , []) :
			emp = emp[0]
			emp_logs = [x for x in logs if x.employee == emp and x.log_time.date() >= self.from_date]
			emp_shifts = get_employee_shifts(emp, self.from_date ,self.to_date, True) or []
			for log in emp_logs :
				log_shift = None
				default_shift = None
				log_date = dateutil.parser.parse(str(log.log_time)).date()
				shifts = [x for x in emp_shifts if log if ((not x.end_date) or (log_date <=  dateutil.parser.parse(str(x.end_date)).date())) ]
				print ("shifts : ",[shift.shift_type.name for shift in emp_shifts ])
				if log.shift and log.shift in [shift.shift_type.name for shift in emp_shifts ]:
					log_shift = get_employee_shift(emp, log_date, False, log.shift)
				doc = frappe._dict()
				if log_shift :
					default_shift = log_shift
				elif len(shifts) > 0:
					default_shift = shifts [-1]
				# elif len(shifts) == 1 :
				# 	default_shift = shifts [0]
				if not default_shift :
							continue
				log.shift = default_shift.shift_type.name
				shift_start , shift_end =  default_shift.start_datetime.time() , default_shift.end_datetime.time()
				log_time = dateutil.parser.parse(str(log.log_time))
				shift_start_date = datetime.combine( log_time.date(), shift_start)
				if default_shift.shift_type.is_24_hours:
						last_day_emp_logs = [x for x in logs if x.employee == emp and  (x.shift_date or x.log_date ) == (shift_start_date.date() + timedelta(days=-1))]

						if len(last_day_emp_logs) > 0:
							# last_log = last_day_emp_logs [-1]
							# if last_log.log_type == '0' :
							log.log_type = "1"
							# else:
								# log.log_type = "0"
						else :
						...
						if log.log_type == "0" :
							log.shift_date = shift_start_date.date()
						elif log.log_type == "1" :
							log.shift_date = shift_start_date.date() + timedelta(days=-1)
				else :
					if shift_start >= shift_end :
						shift_end_date = datetime.combine(log_time.date(), shift_end) + timedelta(days=1)
					else :
						shift_end_date = datetime.combine(log_time.date(), shift_end)

					in_min = abs((log_time - shift_start_date).seconds / 60)
					out_min = abs((log_time - shift_end_date).seconds / 60)
					if in_min >= out_min :
						log.log_type = '0'
					else :
						log.log_type = '1'
				...
				# 	for_date = dateutil.parser.parse(str(log.log_time))
				#
				# 	for_date = timedelta(days=0) if default_shift.shift_type.start_time > shift_type.end_time else for_date
				#
			...
			self.employees_logs [emp] = emp_logs
	...
	def calculate(self,employee , day , flag = None , missions=[] , businessTrips=[] , permissions=[] ):
		...
		doc.is_calculated=0
		user_records_in_day = []
		emp_logs = self.employees_logs.get(employee) or []
		day_logs = [x for x in emp_logs if (x.shift_date or x.log_date) == day]
		if len(day_logs) > 0:
			Check_in = day_logs[0]
			Check_OUT = day_logs[-1]
			user_records_in_day.append(frappe._dict({
				"employee": employee,
				"Day": (Check_in.shift_date or Check_in.log_date),
				"IN": Check_in.log_time,
				"OUT": Check_OUT.log_time,
				"shift": Check_OUT.shift,
			}))
		print(f"employee : {employee}", f"Day : {day} ", "logs : " , user_records_in_day)
		if len(user_records_in_day) == 0 :
			default_shift = get_employee_shift(employee, day, True)

		else :
			default_shift = get_employee_shift(employee, day, True ,user_records_in_day[0].shift )
		...
		if 0 :
			if not doc.is_open_shift and False:
				...
				if doc.is_open_shift and not doc.is_24_hours and False :
					#Open Shift
				...
				elif doc.is_open_shift and  doc.is_24_hours and False :
					# Shift 24 Hours
	...
	def check_attendance_request (self,doc):
			list = frappe.db.sql("""
			select * from `tabAttendance Request` where docstatus = 1 and status = 'Approved'
	...
	def post_attendance(self, conditions=""):
		...
		self.delete_Additional_salary(conditions)
		self.delete_leave_entries(conditions)
			...
			if attendances:
				for attendance in attendances:
					...
					if employee.attendance_rule:
						...
						if attendance_role.enable_less_time_factor_penality:
							...
							if less_penalty_amount and less_penalty_salary_compnent:
								...
								self.submit_Additional_salary(employee.name, less_penalty_salary_compnent,
															  less_penalty_amount, desc, "Less Time Penality" , 1)
						...
						if attendance_role.enable_late_penality:
							...
							if attendance_role.salary_component_for_late_penalty and penality_amount:
								...
								self.submit_Additional_salary(employee.name,
															  ...
															  penality_amount, desc, 'Delays Penality',1)
						...
						if attendance_role.enable_fingerprint_penality:
							...
							if attendance_role.fingerprint_forgetten_penlaity_salary_component and fingerprint_amount:
								...
								self.submit_Additional_salary(employee.name,
															  ...
															  fingerprint_amount, desc, 'Fingerprint Penality',1)
							...
							if attendance_role.absent_rules:
								...
								if absent_rule.senario == 'Deduction from Salary':
									...
									if absent_amount and absents_salary_component:
										...
										add_addiotional_salary = 1
										if absent_rule.deduct_absent_from_leave_balance :
											if absent_rule.leave_type :
												leave_type = frappe.get_doc("Leave Type",absent_rule.leave_type)
												leave_balance = 0

												if not leave_type.allow_negative :

													leave_balance = get_leave_balance_on(employee.name, leave_type.name
																						, self.payroll_start_date, self.payroll_start_date + timedelta(days=absent_amount),
																						consider_all_leaves_in_the_allocation_period=True)
												
												print ("leave_balance ===> " , leave_balance)
												print ("absent_amount ===> " , absent_amount)
												if leave_balance >= absent_amount or leave_type.allow_negative:
													self.submit_leave_ledger(employee.name, leave_type.name , absent_amount)
													add_addiotional_salary = 0


										if add_addiotional_salary :
						...
						# New Hiring Days
						new_hiring_component = frappe.db.get_single_value("Payroll Settings", "new_hiring_component")
						if new_hiring_component :
								date_of_joining = frappe.db.get_value("Employee",attendance.employee , "date_of_joining")
								if date_of_joining :
									date_of_joining = dateutil.parser.parse(str(date_of_joining))
									payroll_start_date = frappe.db.get_value("Payroll Period",self.payroll_period , 'start_date')
									payroll_start_date = dateutil.parser.parse(str(payroll_start_date))
									if payroll_start_date < date_of_joining :
										diff_days = (date_of_joining - payroll_start_date).days
										if diff_days :
											desc = _('New Hiring : {} days'.format(
												str(timedelta(days=diff_days))))
											self.submit_Additional_salary(employee.name, new_hiring_component,
																		diff_days, desc,
																		'New Hiring',1)
	...
	def submit_Additional_salary(self, employee, salary_component, amount, desc , attendance_flag , penality=0):
	...
	def submit_leave_ledger(self, employee, leave_type, leaves):

		last_doc = frappe.db.get_value('Leave Ledger Entry', { "leave_type":leave_type , "employee":employee ,"transaction_type" : self.doctype , "transaction_name" : self.name },  ['name'], as_dict=1)
		if last_doc :
			last_doc = last_doc.name
		...
		if not last_doc :
				...
				doc.from_date = self.payroll_start_date
				doc.to_date = self.payroll_start_date + timedelta(days=abs(flt(leaves)))
				doc.save()
				doc.submit()
		else:
			# doc = frappe.get_doc("Additional Salary",last_doc)
			# doc.amount += amount
			# doc.save()
			frappe.msgprint(_("Employee {employee} has this Leave Ledger Entry {name} before <br>Please delete Leave Ledger Entry {name} it first ".format(employee=employee , name=last_doc)))


	def delete_Additional_salary(self,conditions=""):
		emp_conditions = f"""
		and employee in (
		...
		and salary_slip is null 
		{emp_conditions} 
		""")
	
	
	def delete_leave_entries(self,conditions=""):
		emp_conditions = f"""
		and employee in (
			select name from tabEmployee e 
			where 1 = 1 
		...
		where  transaction_type = '{self.doctype}' and  transaction_name= '{self.name}' 
		{emp_conditions} 
		""")
...
def enqueue_calculate(self, employees):
		...
		for employee in self.employees:
			...
			permissions = frappe.db.sql("""
						select permission.name , date(permission.date) as date , type.code , SUBTIME (permission.to_time,permission.from_time) as Duration
						from tabPermission permission inner join `tabPermission Type` type on type.name = permission.permission_type
						where permission.docstatus = 1 and status = "Approved"
						and date(date) between date('{from_date}') and date('{to_date}')
						and employee = '{employee}'
						group by  permission.name ;
					"""
				.format(from_date = self.payroll_start_date,to_date=self.payroll_end_date , employee = employee),as_dict=1) or []
			...
			missions = frappe.db.sql("""
									select * from `tabMission` where docstatus = 1
									and date(tabMission.date)  between date('{from_date}') and date('{to_date}')
									and employee = '{employee}'
									and status = "Approved"
									order by start_time asc ,  end_time asc
...
def get_employee_shift(employee, for_date, consider_default_shift=False, next_shift_direction=None , shift_type_name = None):
	...
	shift_assignment_details = None
	if shift_type_name :
		shift_assignment_details = frappe.db.get_value('Shift Assignment', {'employee':employee,'shift_type':shift_type_name, 'start_date':('<=', for_date), 'docstatus': '1', 'status': "Active"}, ['shift_type', 'end_date','working_hours','start_time','end_time'])
	if not shift_assignment_details :
...
def get_employee_shifts(employee, from_date , to_date, consider_default_shift=False, next_shift_direction=None):
	"""Returns a Shift Type for the given employee on the given date. (excluding the holidays)
	...
	# 	shift_assignment_details = frappe.db.get_value('Shift Assignment', {'employee':employee,'shift_type':shift_type_name, 'start_date':('<=', for_date), 'docstatus': '1', 'status': "Active"}, ['shift_type', 'end_date','working_hours','start_time','end_time'])
	# if not shift_assignment_details :
	# shift_assignment_details = frappe.db.get_value('Shift Assignment', {'employee':employee, 'start_date':('<=', from_date), 'docstatus': '1', 'status': "Active"}, ['shift_type', 'end_date','working_hours','start_time','end_time'])
	shifts = frappe.db.sql(f"""
	select shift_type, end_date,working_hours,start_time,end_time ,start_date from `tabShift Assignment`
		where status ='Active' and docstatus = 1 and date('{from_date}') >= start_date and (ifnull(end_date,'')='' or date('{to_date}') <= end_date)
		and employee='{employee}' ;
	""") or []
	employee_shifts = []
	employee_shifts_names = []
	for shift_assignment_details in shifts :
		if shift_assignment_details:
			shift_type_name = shift_assignment_details[0]
			working_hours = shift_assignment_details[2] or 0
			start_time = shift_assignment_details[3]
			...
			# if end_date present means that shift is over after end_date else it is a ongoing shift.
			if shift_assignment_details[1] and from_date >= shift_assignment_details[1] :
				shift_type_name = None
		...
		if not shift_type_name and consider_default_shift:
			...
			if not shift_type_name:
				...
				if company :
					shift_type_name = frappe.db.get_value('Company', company, 'default_shift')
		if shift_type_name and shift_type_name not in  employee_shifts_names :

			shift_details =  get_shift_details(shift_type_name, from_date , working_hours,start_time,end_time)
			shift_details.start_date = shift_assignment_details[5]
			shift_details.end_date= shift_assignment_details[1]
			employee_shifts.append(shift_details)
			employee_shifts_names.append(shift_type_name)
