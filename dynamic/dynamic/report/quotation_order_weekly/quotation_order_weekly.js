...
frappe.query_reports["Quotation Order Weekly"] = {
	"filters": [
		{
			fieldname: "range",
			label: __("Range"),
		...
		// },
		{
			...
			label: __("based_on"),
			fieldtype: "Select",
			options: ["Sales Order","Quotation","Sales Invoice"],
			default: "Quotation",
			reqd: 1
		},
		// {
		...
		// 		var applicant_type = frappe.query_report.get_filter_value('doc_type');
		// 		var applicant = frappe.query_report.get_filter_value('docname');
