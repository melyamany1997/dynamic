...
def get_rootwise_opening_balances(filters, report_type):
	...
	gle = frappe.db.sql(
		"""
		select
			account, sum(debit) as opening_debit, sum(credit) as opening_credit,posting_date
		from `tabGL Entry`
		where
			company=%(company)s
			{additional_conditions}
			and (posting_date Between %(from_date)s AND  %(to_date)s or ifnull(is_opening, 'No') = 'Yes')
...
def accumulate_values_into_parents(accounts, accounts_by_name):
	# frappe.errprint(f'accounts-->{accounts}')
