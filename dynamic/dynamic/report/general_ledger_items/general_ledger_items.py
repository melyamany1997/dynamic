...
def execute(filters=None):
	...
	# first check if fiters . party and party type
	if filters.get('party_type'): #and filters.get('party'):
		res = get_transactions_items(res)
		
	return columns, res

def get_transactions_items(result):
	new_data = []
	for row in result:
		new_data.append(row)
		if row.get('voucher_no') and row.get('voucher_type'):
			...
			if row.get('voucher_type') == "Sales Invoice":
				...
				query_data = frappe.db.sql(sql_query,as_dict=1)
				new_data.extend(query_data)
			# frappe.errprint(f'new_data-->{new_data}')
	return new_data
